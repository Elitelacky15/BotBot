...
module.exports = class extends Event {
    ...
    async run(message) {
        try {
            ...
            if (message.content.match(mentionRegex)) {
                ...
                message.channel.send(`Hello there! My prefix is ${config.prefix}`).then((s) => {
                    s.delete({
                        timeout: 10000
                    })
                }).catch(() => {})
            ...
            if (command) {
                ...
                if (typeof rateLimit === "string") return message.channel.send(`${message.author} Please wait **${rateLimit}** before running the **${cmd}** command again!`);
                ...
                if (command.botPermission) {
                    ...
                    if (missingPermissions.length !== 0) {
                        const embed = new MessageEmbed()
                            .setAuthor(`${this.client.user.tag}`, message.client.user.displayAvatarURL({
                                dynamic: true
                            }))
                ...
                if (command.disabled) return message.channel.send(`**This command is currently disabled by the owner!**`)
                ...
                await this.runCommand(message, cmd, args)
                    ...
                    .catch(error => {
                        console.log(error)
                        return message.channel.send(`**A fatal error occured while executing the command please contact developers.**`)
                    })
            }
        } catch (error) {
            console.log(error)
    ...
    ratelimit(message, cmd) {
        try {
            ...
            console.log(e)
            message.channel.send(`**A fatal error occured while executing the command please contact developers.**`)
        }
    }
}